c--ZONA NORTE
        ...
        PARAMETER(M=149) !altoentro=75
        integer O(N,M) !decide quÈ pÌxeles son parte o no
        ...
        jcentro=75
        ancho=1.454 !ancho en grados entre 2 apuntados
        chi=-50. ! Center of the field = Declination JPAS=55
        a0=140. ! Center of the field = RA + 6h	JPAS=285
        !chi=55.
        !a0=285.
        raN=15
        decN=35
...
c--------------------------------------------------------------------|
     ...
     &(dec.lt.-70.)) then
              O(i,j)=-1
			            ...
			            if((dec.gt.-18.).and.(ra.lt.180.)) then
              O(i,j)=-1
            endif
            if((dec.gt.-18.).and.(ra.lt.320.)) then
              O(i,j)=-1
            endif
            if((ra.gt.105.).and.(ra.lt.295.)) then
              O(i,j)=-1
            endif
            if((dec.lt.-60.).and.((ra.gt.105.).and.(ra.lt.300.))) then
              O(i,j)=-1
            endif
			if((dec.gt.-45).and.((ra.gt.65).and.ra.lt.240)) then
				O(i,j)=-1
			endif
			if((dec.gt.-35).and.((ra.gt.40).and.ra.lt.240)) then
				O(i,j)=-1
			endif
...
c--calculo lÌmites
        ...
        open(11,file='splus_pointsul.dat')
        do j=1,M
        do i=1,N    
         if(O(i,j).eq.1) then
