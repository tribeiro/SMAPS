...
if __name__ == '__main__':
    ...
    ectobs.sort()
    ectobs = np.arange(14,35,0.1)

    ecflx = mag0 - ecbinary(tt, 2, 0.1, 1.5) + 1
    ecobs = mag0 - ecbinary(ectobs, 2, 0.1, 1.5) + 1
    ecerr = np.random.exponential(0.1, len(ectobs)) * (-1) ** np.random.randint(0, 2, len(ectobs))

    star = mag0 + np.zeros(len(ectobs)) + np.random.exponential(0.1, len(ectobs)) * (-1) ** np.random.randint(0, 2, len(ectobs))
    ...
    py.errorbar(ectobs, star, 0.1, fmt='o')

    py.ylim(18.499, 15.5)

    ax1 = py.subplot(111)

    bb = np.linspace(np.min(ecobs + ecerr),np.max(ecobs + ecerr),101)

    py.hist(ecobs + ecerr,bins=bb)
    py.hist(star,bins=bb)
